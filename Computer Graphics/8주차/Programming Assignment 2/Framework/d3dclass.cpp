...
// Setting wireframe func. Call from external func.
void D3DClass::SetWireFrame(bool wireframe)
{
	m_wireframeOn = wireframe;
	UpdateRasterState();
}

void D3DClass::ToggleCulling()
{
	m_cullingOn = !m_cullingOn;
	UpdateRasterState();
}

void D3DClass::UpdateRasterState()
{
	D3D11_RASTERIZER_DESC rasterDesc;
	rasterDesc.AntialiasedLineEnable = false;
	rasterDesc.FillMode = m_wireframeOn ? D3D11_FILL_WIREFRAME : D3D11_FILL_SOLID;
	rasterDesc.CullMode = m_cullingOn ? D3D11_CULL_BACK : D3D11_CULL_NONE;
	rasterDesc.DepthBias = 0;
	rasterDesc.DepthBiasClamp = 0.0f;
	rasterDesc.DepthClipEnable = true;
	rasterDesc.FrontCounterClockwise = false;
